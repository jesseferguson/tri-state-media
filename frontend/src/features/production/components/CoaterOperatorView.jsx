...
import DeleteMaterialRollDialog from "../../materials/components/DeleteMaterialRollDialog";
...
export default function CoaterOperatorView({ currentUser, initialMaterialScheduleId = "", onInitialMaterialScheduleHandled, linkedRollTagId = "", onLinkedRollTagChange, onLinkedRollTagClose }) {
  ...
  const dataQuery = useQuery({
    ...
    queryFn: async () => {
      const [tags, schedule, materials, supplierOptions, presses, locations, rawMaterials, skids] = await Promise.all([
        fetchCollection("coater-roll-tags", { ordering: "press__name,press_sequence,run_date,tag_number", pageSize: 500, fetchAll: true }),
  ...
  useEffect(() => {
    if (!initialMaterialScheduleId || dataQuery.isLoading) return;
    const schedule = data.tags.find((tag) => sameId(tag.id, initialMaterialScheduleId) && !tag.source_schedule);
    if (!schedule) return;
    if (schedule.press) {
      setSelectedPress(String(schedule.press));
      savePressPreference(currentUser, String(schedule.press));
    }
    setSelectedTagId(String(schedule.id));
    setSelectedProductId("");
    onInitialMaterialScheduleHandled?.();
  }, [currentUser, data.tags, dataQuery.isLoading, initialMaterialScheduleId, onInitialMaterialScheduleHandled]);
  ...
  const materialJobs = useMemo(() => data.tags
    ...
    .sort((a, b) => Number(a.press_sequence || 9999) - Number(b.press_sequence || 9999) || String(a.run_date || "").localeCompare(String(b.run_date || "")) || String(a.tag_number || "").localeCompare(String(b.tag_number || ""))),
