import { AlertTriangle, CheckCircle2, CircleDollarSign, Copy, Download, FileText, Image as ImageIcon, Layers3, Mail, MessageSquarePlus, MoreHorizontal, Pencil, Plus, Printer, Ruler, Search, SlidersHorizontal, Trash2, XCircle } from "lucide-react";
...
export default function QuotePricingTool({
  currentUser,
  initialJobTicketId = "",
  initialCustomerId = "",
  canManageQuoteMaterials = false,
  canApproveQuotes = false,
  onStartCustomerFollowUp = null,
}) {
  ...
  function startCustomerFollowUp(quote) {
    if (!quote || typeof onStartCustomerFollowUp !== "function") return;
    const customer = customers.find((item) => String(item.id) === String(quote.customerId || quote.customer_id || ""));
    if (!customer?.id) {
      setQuoteActionError("Link this quote to a customer before creating a follow-up.");
      return;
    }
    setQuoteActionError("");
    onStartCustomerFollowUp({ quote, customer });
  }
          ...
          <div className="quote-record-view quote-panel">
            <div className="quote-record-actions">
              ...
              <div className="quote-record-action-line">
                ...
                <button className="quote-email-action" type="button" onClick={() => startCustomerFollowUp(selectedQuote)} disabled={!selectedQuote}>
                  <MessageSquarePlus size={15} /> <span>Follow Up</span>
                </button>
              </div>
              <details className="quote-record-action-menu">
                ...
                <div>
                  ...
                  <button type="button" onClick={() => startCustomerFollowUp(selectedQuote)} disabled={!selectedQuote}><MessageSquarePlus size={15} /> Create Follow-Up</button>
