...
def skid_label_zpl(skid, scan_url, *, darkness="20", speed="5", copies=1):
    return "\n".join([
        ...
        f"{STORAGE_QR_ORIGIN}{STORAGE_QR_COMMAND}^FDLA,{zpl_text(scan_url)}^FS",
        "^FO30,566^A0N,23,23^FB549,1,0,C^FDSCAN FOR LIVE CONTENTS^FS",
...
def rack_label_zpl(rack, scan_url, *, darkness="20", speed="5", copies=1):
    return "\n".join([
        ...
        f"{STORAGE_QR_ORIGIN}{STORAGE_QR_COMMAND}^FDLA,{zpl_text(scan_url)}^FS",
        "^FO30,566^A0N,23,23^FB549,1,0,C^FDSCAN TO MOVE SKIDS^FS",
        f"^PQ{zpl_copies(copies)}",
        "^XZ",
    ])
